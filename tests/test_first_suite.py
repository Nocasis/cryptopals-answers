...
class TestFirstSuite:
    ...
    def test_haming(self):
        assert (cryplib.haming(b"this is a test", b"wokka wokka!!!") == 37)

    # Challenge 6.2
    def test_attack_repeatingxor(self):
        with open("res/6.txt", "r") as f:
            result = cryplib.attack_repeatingxor(f.read())
    ...
    def test_aes_ecb(self):
        ...
        decrypted_data = cryplib.aes_decrypt_ecb(data, key, pad=True)
        assert (decrypted_data == plain)
        assert (cryplib.aes_encrypt_ecb(plain, key) == data)

    # Challenge 8
    def test_detect_aes_ecb(self):
        encrypted_strings = list(map(bytes.fromhex, open("res/8.txt", "r").read().split('\n')[:-1]))
        result = cryplib.detect_aes_ecb_simple(encrypted_strings)
